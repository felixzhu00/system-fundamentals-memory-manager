...
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	//Check precondition of memalign
	//"alignment is a power of two" & "least as large as the default alignment"
	sf_errno = 0;
...
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	//memalign should do what malloc can do
...
Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {
	//call memalign where a larger address within the block that satisfies the requested
	...
	sf_block *bp = (sf_block *)(x-8);
	// unsigned long payload_address_align = ((unsigned long)(bp->body.payload)) & (align-1);
	// unsigned long  correct_size = ((bp->header)&~0x7);

	cr_assert(((uintptr_t)x)%40 == 0, "Payload address is not aligned!");
	cr_assert((bp->header & ~0x7) == 48, "Block size is not expected");
